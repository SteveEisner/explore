...
| Voice front-end screenshot tool: "snapshot send was too large for the channel" | voice | seen live 2026-07-12 during D6 verification — the WebRTC data-channel payload path needs chunking or downscaling; unrelated to the envelope change. Once fixed, revisit the parked idea of attaching a screenshot to every voice turn so deictic references resolve visually (weigh tokens/latency) |
...
| Vault meta relocation: Steve wants the vault-generated meta docs at `docs/design/api/`, but markdown-vault-mcp regenerates them at `docs/meta/` (observed 2026-07-12 20:18 — a manual move goes stale immediately). Find a vault config option for the meta directory, or wrap; until then `docs/meta/` stays gitignored in place | wiki | from The Optimizer's preload fix |
| Chat pane anchors to the bottom like a real chat: stay pinned to the newest message as content streams in (auto-scroll on new/updating messages, but don't yank if the user has scrolled up to read) | client | from Steve 2026-07-12 |
| Wiki rename/move (and maybe delete) as a capability | wiki | from the 2026-07-12 document-capability parity audit: the original Wiki API design lists rename, but no surface exposes it today — not the app UI, not the voice tools, not the Claude session. If it lands, follow the create_doc pattern: one wiki-service function, exposed as a voice server tool + a wiki-MCP tool |
| Indicate-tool highlight should take a lot longer to fade away: the scroll-to-and-blink emphasis on on-screen content disappears too fast to register — lengthen the fade-out (and maybe the hold) | client | from Steve 2026-07-12 |
| "Invalid artifact filename" hit in live use (Steve, 2026-07-12 — no repro details captured): investigate the save-as-.oui filename path — likely either the name normalization/hostile-name rejection firing on a reasonable name, or a generation-supplied default name failing validation; make the error say *why* the name is invalid either way | client/server | needs repro; check artifact:save normalization rules first |
| oui-embed same-origin check misses protocol-relative URLs: `src.includes("://")` rejects `https://evil.com/x` but passes `//evil.com/x`, which then reads as site-absolute and fetches cross-origin — resolve via `new URL(src, location.origin)` and compare origins (the header comment "Cross-origin sources are refused" should then hold) | client | from the Cleaner's 2026-07-12 clarity review; file left untouched because the expanded-artifact work has it in flight |
| `createDoc` polish (wiki-service.ts): (1) its teaching errors name the voice tools ("edit_doc for text, edit_artifact for .oui") but the same errors reach the CLI model via wiki-MCP `create_file`, whose surface has neither tool (it has the vault edit tool + `edit_artifact` under different names) — make the message surface-neutral or per-caller; (2) empty content is rejected by the voice tool schema (`min(1)`) but accepted by MCP `create_file` and the service itself (writes a lone `"\n"`) — put the boundary in the service once; (3) the existsSync-then-write pair races — `writeFile(..., { flag: "wx" })` makes create-exclusive atomic and drops the pre-check | server | from the Cleaner's 2026-07-12 clarity review of the backend |
...
| 12. Stream delegation progress: broadcast `chat:tool` at `content_block_start` (name available in stream events) instead of assistant-message completion; forward thinking block start/stop as a status pulse; browser injects progress into the voice conversation so it can narrate instead of dead air. Measured silent gaps 3.1–9.6s even on 4–12s jobs, cover signals exist within ~2s ([eval/agent-timing-report.md](eval/agent-timing-report.md) §b) | J3 | — | todo |
| 13. Delegated wiki writes land in the sandbox: 2/4 default runs used `Write` (file in `sandbox/`, not the wiki) while reporting success — teach the delegation prompt to use the wiki tools for wiki files (or disallow bare `Write` on delegated jobs) and verify placement before declaring success ([eval/agent-timing-report.md](eval/agent-timing-report.md), correctness section) | J3/J2 | — | todo |
