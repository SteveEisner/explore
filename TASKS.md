...
| Optimize UI-interaction time: grind on the biggest contributors the performance eval identifies — per [eval/sweep-report.md](eval/sweep-report.md): (1) default generation model → Opus 4.8 / Sonnet 5, (2) grounded wiki-read cost (~2.5s; consider preloading likely pages at connect), (3) time-to-first-token is the dominant remaining slice (model-side); skip effort/speed-hint (nulls) and the post-UI tail (off critical path) | J3 | The Optimizer | todo |
