...
| Calls to Claude take WAY too long in live use — DIAGNOSED 2026-07-13 (main session) from a real 47.6s ask_artifact_agent delegation: (a) 64% was the model typing one huge tool call (full-artifact rewrite as a single vault line_replace, ~3K output tokens at ~100 tok/s, stream continuous — enforcing incremental region edits / a faster tier for bulk rewrites is the lever); (b) the other ~17s is context weight — the live session carries cache_read_input_tokens ≈ 1.07M, so every turn pays ~5-6s of prompt pass vs the harness's fresh ~25K sessions (session restart/compaction reclaims this; also explains cumulative cost $8.92). Remaining work: session compaction/restart policy + incremental-edit enforcement | J3 | main session | diagnosed |
| Enable auto-compact on the backend CLI session: headless --print mode has no explicit /compact (verified v2.1.207 — no slash command, no control message, no SDK API), but auto-compact runs in all modes and simply never fired because the session never neared the model window. Pass `CLAUDE_CODE_AUTO_COMPACT_WINDOW` (~200K) + `CLAUDE_AUTOCOMPACT_PCT_OVERRIDE` (~50) in claude.ts's spawn env so the session self-compacts around ~100K tokens; VERIFY the env var names against the installed binary first (doc-sourced). Note: each compaction rewrites the transcript prefix → one full cache re-create when it fires (fine if rare). `--fork-session` is the cleaner rotation primitive (resume old context under a new id) vs today's manual move-aside | J3 | — | todo |
| **Cache-miss economics (diagnosed 2026-07-13, main session):** 5-day log totals $335.98; $161 of it is 63 full cache rebuilds, and $125.79 went to 52 pre-warm turns that produced ≤10 output tokens while re-caching the whole 150-400K transcript. Cause chain: the appended system prompt embeds the live D7 wiki preload and is rebuilt per spawn → any respawn (tsx watch server restarts, model-switch delegations) changes the prompt prefix within ~8K tokens → prefix-keyed cache invalidates → next turn re-creates everything; the connect-time pre-warm then pays that bill on every browser reload. Fixes: (1) stabilize the prefix — move the wiki preload out of the per-spawn system prompt (pinned first message, or per-session frozen snapshot); (2) pre-warm only when the cache would hit (byte-identical prompt) ; (3) session rotation/compaction — session 57f3d8f2 is 5 days / ~127 turns / ~350-460K-token transcript, which also costs ~5-6s of restore per API call even on clean hits | J3 | — | todo |
...
| Task | Area | Notes |
|---|---|---|
| Screenshot paths ignore an expanded artifact | client | done 2026-07-12 (main session): all capture/observe paths (chat screenshot, voice take_screenshot, state tool, indicate) now target the active surface — the expanded overlay when one is up, the document otherwise; verified via the state provider reporting the overlay's scroll while expanded |
...
| **D8 renderer: inline artifacts.** Render markdown fenced code blocks with language `oui` as embedded artifacts (same preview treatment as `<oui-embed>`, hot-reload included) — done 2026-07-12 (main session): ```oui fences on wiki pages render as launchable previews addressed by {doc, fence line}; chat messages keep plain code blocks | 4 | main session | done |
| **D8 maximize for inline artifacts.** Done 2026-07-12 (main session): `app/expanded-artifact` accepts {doc:'/docs/page.md', line:N} alongside .oui URLs; the expanded view re-reads the block from the page (hot-reloads with it); voice expand_artifact takes .md pages (first block); every project .oui migrated inline and deleted | 4 | main session | done |
| Pack OUI into markdown files instead of a separate .oui format? (Steve, 2026-07-12) — decided same day as **D8** (decisions.md, The Optimizer): canonical artifact is an OpenUI block inline in a markdown doc, carried as a fenced ```oui code block; separate .oui files demoted to reuse/legacy. Remaining implementation lives in the two D8 renderer rows directly above | 4 | The Optimizer | done |
...
| 15. (P2) Direct `create_doc` voice tool: the registry has no create — the front-end-scenario suite (`eval/frontend-scenarios.ts`) had to delegate a purely mechanical file write to the backend (6.5s, $0.29) where `wiki-service.createDoc` would answer in milliseconds (it already powers the backend's `mcp__wiki__create_file`, parent dirs included). Consider rename/delete parity while there (delete behind a confirm) | J3 | — | todo |
| 16. (P2) `edit_doc`'s missing-file teaching error dead-ends: "does not exist — check list_docs" gives a voice model no path to *create* the file; extend the error to name the create path (delegation today, the row-15 tool once it exists) | J3 | — | todo |
| 17. (P2) "Create a folder" has no direct answer: directories exist only implicitly via file creation (works — `create_file` makes parents), `list_docs` lists files only, empty dirs are unrepresentable. Add a line to voice-agent.md so the model explains-and-creates (e.g. seeds a README) instead of flailing | J2/J3 | — | todo |
| 18. (P2) Delegated jobs pay a ~2s `list_files` verification round trip (the row-13 preamble) on every wiki-writing job — correct tradeoff today (it caught real failures), but once row 15 gives the front end cheap direct writes, most mechanical creations shouldn't be delegated at all; re-examine the preamble's verify-always rule then | J3 | — | todo |
