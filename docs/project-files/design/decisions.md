...
```oui
root = Stack([styles, header, deck, footer], "dd", null, [{key: "deck/slide", initial: 0, description: "Current slide in the decisions deck"}])

styles = Content("<style>\n.stack.dd { --ink:#14181f; --ink2:#54606e; --dim:#95a0ad; --line:#e3e7ec; --panel:#f7f8fa; --accent:#2f6f8f; font-family:system-ui,-apple-system,'Segoe UI',Roboto,Arial,sans-serif; line-height:1.6; color:var(--ink); background:#fff; padding:22px 24px; }\n.stack.dd h1,.stack.dd h2 { margin:0; line-height:1.2; }\n.stack.dd p { margin:0; color:var(--ink2); }\n.stack.dd ul { margin:10px 0 0; padding-left:18px; }\n.stack.dd li { margin:0 0 7px; color:var(--ink2); font-size:14px; }\n.stack.dd li b { color:var(--ink); font-weight:600; }\n.dd-head { border-bottom:1px solid var(--line); padding-bottom:14px; margin-bottom:14px; }\n.dd-head .eye { font-size:10.5px; letter-spacing:.16em; text-transform:uppercase; color:var(--accent); font-weight:700; margin-bottom:7px; }\n.dd-head h1 { font-size:23px; letter-spacing:-.01em; }\n.dd-head p { font-size:14px; margin-top:5px; }\n.tabs.dd-tabs .tabs-nav { display:flex; gap:4px; flex-wrap:wrap; margin-bottom:16px; }\n.tabs.dd-tabs .tabs-trigger { padding:6px 14px; font-size:12px; font-weight:700; letter-spacing:.04em; color:var(--ink2); cursor:pointer; background:var(--panel); border:1px solid var(--line); border-radius:5px; }\n.tabs.dd-tabs .tabs-trigger:hover { color:var(--ink); border-color:var(--accent); }\n.tabs.dd-tabs .tabs-trigger.active { background:var(--accent); border-color:var(--accent); color:#fff; }\n.slide { border:1px solid var(--line); border-radius:9px; padding:20px 22px; min-height:186px; }\n.slide .n { font-size:10.5px; letter-spacing:.14em; text-transform:uppercase; color:var(--accent); font-weight:700; margin-bottom:6px; }\n.slide h2 { font-size:19px; }\n.dd-foot { margin-top:16px; padding-top:12px; border-top:1px solid var(--line); font-size:12px; color:var(--dim); }\n</style>")

header = Content("<div class='dd-head'><div class='eye'>Decisions deck</div><h1>Eight decisions, newest first</h1><p>Each one recorded with a rationale and the conditions that would make us revisit it. Flip through with the tabs.</p></div>")

deck = Tabs([{label: "D8", content: [s8]}, {label: "D7", content: [s7]}, {label: "D6", content: [s6]}, {label: "D5", content: [s5]}, {label: "D4", content: [s4]}, {label: "D3", content: [s3]}, {label: "D2", content: [s2]}, {label: "D1", content: [s1]}], "deck/slide", "dd-tabs")

s8 = Content("<div class='slide'><div class='n'>D8 · 2026-07-12</div><h2>Artifacts live inline in markdown documents</h2><ul><li>The canonical artifact is an <b>OpenUI block inside a markdown page</b>, not a separate <code>.oui</code> file — carried in a fenced <code>oui</code> code block.</li><li><b>The document is the unit of thought:</b> prose and interactive views travel together, in reading order.</li><li><b>Editing collapses to one surface</b> — the block is just text, so the same wiki edit tools patch it as patch the prose.</li><li>Separate <code>.oui</code> files remain supported for existing artifacts and cross-document reuse.</li></ul></div>")

s7 = Content("<div class='slide'><div class='n'>D7 · 2026-07-12</div><h2>Preload the wiki; Opus 4.8 as the default model</h2><ul><li>Small root-level wiki pages are <b>inlined into the system prompt</b> at spawn, with an instruction to use that copy rather than re-read.</li><li>Grounded ask→UI dropped <b>7.7s → 4.4s</b>; the wiki-read round trip disappears (3 turns → 2), byte-exact and slightly cheaper.</li><li>Whole files only, smallest-first, under a budget — a truncated page is a grounding hazard.</li><li><code>claude-opus-4-8</code> becomes the explicit default model.</li></ul></div>")

s6 = Content("<div class='slide'><div class='n'>D6 · 2026-07-11</div><h2>One feedback envelope for every channel</h2><ul><li>Chat, voice, screenshot, drawing and point-and-comment all arrive in <b>one shape</b>, not four ad-hoc formats.</li><li>A <b>state snapshot is always attached</b>, which makes feedback self-locating — &ldquo;this table is confusing&rdquo; is ambiguous without knowing what was on screen.</li><li>The chat pane renders one message type with optional adornments instead of per-channel kinds.</li></ul></div>")

s5 = Content("<div class='slide'><div class='n'>D5 · 2026-07-11</div><h2>A realtime voice agent as a second intelligence</h2><ul><li>Browser-direct <b>WebRTC audio</b> to a realtime model; the server only mints ephemeral tokens and never relays the audio.</li><li>Tools are bridged over the <b>existing websocket</b> — wiki, artifact and state edits, plus delegation of heavy work to the builder agent.</li><li>Toggle-mic open conversation: the voice model drives the talk, the artifact agent does the building.</li></ul></div>")

s4 = Content("<div class='slide'><div class='n'>D4 · 2026-07-07</div><h2>Structural components are named editing points</h2><ul><li>A component earns its place over raw HTML only by offering an advantage — for structural ones that is <b>a boundary that can be edited independently</b>.</li><li>So they carry <b>no default styling and no behavior</b>; appearance comes from content and artifact stylesheets.</li><li>Behavior must be earned — Gallery selection and context gating exist because structure alone can't provide them.</li></ul></div>")

s3 = Content("<div class='slide'><div class='n'>D3 · 2026-07-07</div><h2>Component state lives in a hierarchical key-value store</h2><ul><li>All interactive state is driven from one central store; artifacts <b>declare their keys up front</b> in a manifest.</li><li>The payoff: <b>a host-driven change takes exactly the same path as a user click</b> — so the model can steer the UI.</li><li>It also makes feedback state-taggable, interaction signals observable, and UI position serializable for free.</li></ul></div>")

s2 = Content("<div class='slide'><div class='n'>D2 · 2026-07-07</div><h2>OpenUI component vocabulary as the artifact medium</h2><ul><li>Artifacts are composed from a <b>constrained vocabulary</b> rather than free-form HTML — the library is the contract for what the model may emit.</li><li><b>Editability is the crux:</b> a structured, line-oriented representation makes revision a small edit, not surgery on an HTML blob.</li><li>A raw-HTML escape hatch remains; leaning on it is the signal to grow the vocabulary.</li><li>Status: <b>experimental</b> — an experiment in defining an HTML-editing protocol for LLMs.</li></ul></div>")

s1 = Content("<div class='slide'><div class='n'>D1 · 2026-07-07</div><h2>Wiki edits use exact search/replace</h2><ul><li>Edits are expressed as <b>exact string search and replace</b> (<code>str_replace</code> style) — never line numbers or unified diffs.</li><li>Benchmarks put it <b>23–27pp ahead</b> of diff and line-number patching, and it matches what the models are trained on.</li><li>The same instinct that drives D2: make the edit format the reliable part.</li></ul></div>")

footer = Content("<div class='dd-foot'>A quick presentation view of this page. The full decision records — rationale, consequences, and revisit conditions — follow below in the document.</div>")
```
